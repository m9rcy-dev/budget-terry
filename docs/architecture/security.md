...
- **Email delivery is provider-swappable by design**: `apps/api/src/mail/mail-provider.interface.ts` defines a `MailProvider` interface, selected via `MAIL_PROVIDER` and constructed by a factory in `mail.module.ts` with an exhaustiveness (`never`) check — adding a provider without a matching `switch` case fails the build. Two implementations exist: `SmtpMailProvider` (nodemailer), used locally against Mailpit (`docker-compose.yml`, no auth), and `ResendMailProvider` (Resend's HTTP API, `RESEND_API_KEY`), the production provider (`MAIL_PROVIDER=resend`). Resend was chosen over MailerLite specifically because MailerLite is a marketing-email platform, not built for transactional mail like a login code. Switching provider again later, or adding a third, is a config change plus one new `MailProvider` implementation — `AuthService` and every other caller only ever see `MailService`. **Not yet done**: wiring a real `RESEND_API_KEY` into a deployed environment — that's Phase 14 (Deployment) scope, tracked in Known Issues below, not this work.
