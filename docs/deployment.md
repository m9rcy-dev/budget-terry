...
3. Deploy. First deploy runs `pnpm install`, `prisma generate && nest build` (via the `build` script), then `prisma migrate deploy` (folded into `buildCommand` — Render's free tier doesn't support `preDeployCommand`, the more natural spot for this — applying every migration in `apps/api/prisma/migrations/` to the fresh Neon database), before starting the service.
